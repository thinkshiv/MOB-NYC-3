...
public class GroceryList {
    ...
    public func flattenedMyCart() -> [FlatGroceryItem] {
    ...
    private let savedMyCartPath: String = {
        let paths = NSSearchPathForDirectoriesInDomains(.DocumentDirectory, .UserDomainMask, true)
        let docPath = paths.first as! String
        return "\(docPath)/com.rw.iCook.MyCart.json"
        }()
    ...
    private func syncedGroceryItems() -> GroceryItems {
        if let data = NSData(contentsOfFile: savedMyCartPath) {
            if let rawMyCart = NSKeyedUnarchiver.unarchiveObjectWithData(data) as? GroceryItems {
                return rawMyCart
            }
        }
        return GroceryItems()
    ...
    private func saveCurrentState() {
        ...
        if !NSFileManager.defaultManager().createFileAtPath(savedMyCartPath, contents: data, attributes: nil) {
